...
library OracleLibrary {
    ...
    function transform(
        ...
        unchecked {
            ...
            if (tickDelta > maxAbsTickDelta || tickDelta < -maxAbsTickDelta) {
                truncatedTick =
                    last.prevTruncatedTick + (tickDelta > 0 ? maxAbsTickDelta : -maxAbsTickDelta);
            }
            ...
            return Observation({
                ...
                tickCumulativeTruncated: last.tickCumulativeTruncated
                    + int56(truncatedTick) * int56(uint56(timeDelta)),
                initialized: true
            });
        }
    }
    ...
    function observeSingle(
        ...
        unchecked {
            ...
            getSurroundingObservations(
                self, time, target, tick, index, cardinality, maxAbsTickDelta
            );
    ...
    /// @notice Returns the accumulator values as of each time seconds ago from the given time in the array of `secondsAgo`.
    /// @dev Reverts if `secondsAgo` > oldest observation.
    /// @param self The stored oracle array
    /// @param time The current block.timestamp
    /// @param secondsAgo Each amount of time to look back, in seconds, at which point to return an observation
    /// @param tick The current tick
    /// @param index The index of the observation that was most recently written to the observations array
    /// @param cardinality The number of populated elements in the oracle array
    /// @return tickCumulative The tick * time elapsed since the pool was first initialized, as of each `secondsAgo`
    ...
    function observe(
        ...
        internal
        view
        returns (int56[] memory tickCumulative, int56[] memory tickCumulativeTruncated)
    {
        unchecked {
            tickCumulative = new int56[](secondsAgo.length);
            tickCumulativeTruncated = new int56[](secondsAgo.length);
            for (uint256 i = 0; i < secondsAgo.length; i++) {
                (tickCumulative[i], tickCumulativeTruncated[i]) = observeSingle(
                    self, time, secondsAgo[i], tick, index, cardinality, maxAbsTickDelta
                );
            }
        }
    }
